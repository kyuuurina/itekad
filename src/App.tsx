...
export default function ItekadMobileApp() {
  const steps = [
    { key: "apply", title: "Identity & Basics", icon: "📱", shortTitle: "Identity" },
    { key: "docs", title: "Documents Upload", icon: "🗂️", shortTitle: "Documents" },
    { key: "vet", title: "Auto Vetting", icon: "🤖", shortTitle: "Vetting" },
    { key: "prep", title: "Interview Prep", icon: "🎓", shortTitle: "Prep" },
    { key: "onboard", title: "Onboarding & e‑Invoicing", icon: "📊", shortTitle: "Onboard" },
    { key: "profile", title: "Financial Profile", icon: "📋", shortTitle: "Profile" },
  ...
  const [docs, setDocs] = useState({ ic: false, ssm: false, bank3m: false, plan: false, photo: false });
  const docsScore = useMemo(() => Object.values(docs).filter(Boolean).length, [docs]);
  ...
  const decision = useMemo(() => {
    ...
    if (ccris === "minor" || ctos === "issue" || thinFile) {
      ...
      return { outcome: "FLAG", reasons: reasons.length ? reasons : ["Manual review needed"] };
    }
  ...
  // STEP 5: e‑Invoicing
  const [entries, setEntries] = useState<Array<{id: number, type: string, label: string, amount: number}>>([]);
  ...
  const addEntry = () => {
    if (!label || !amt) return;
    setEntries(prev => [...prev, { id: Date.now(), type: etype, label, amount: Math.abs(amt) }]);
    setLabel("");
    setAmt(0);
  };
  ...
  const scamFindings = useMemo(() => {
    ...
    if (/transfer|bayar|yuran|fee/.test(lower) && /proses|processing/.test(lower)) hits.push("Asking for upfront fee");
    if (/klik|click|link|pautan/.test(lower)) hits.push("Suspicious link present");
    if (/akaun.*disekat|account.*suspend|blok/.test(lower)) hits.push("Account blocking threat");
    if (/whatsapp.*agent|pegawai.*bank/.test(lower)) hits.push("Impersonation of bank staff");
    return hits;
  }, [sms]);
        ...
        {stepIdx === 0 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">{steps[0].title}</h2>
              <p className="text-sm text-gray-600 mt-1">Verify your identity with e-KYC</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">MyKad / IC (12 digits)</label>
                <input 
                  value={ic} 
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg text-xs text-blue-800">
              💡 This simulates MyDigitalID verification: selfie + IC match, liveness check, and sanctions screening.
            </div>
          </MobileCard>
        )}
        ...
        {stepIdx === 1 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">Upload required documents</p>
            </div>
            ...
            <div className="space-y-3">
              {Object.entries({ 
                ...
                <label key={key} className="flex items-center justify-between p-3 bg-gray-50 border rounded-lg active:bg-gray-100 transition">
                  <div className="flex items-center">
                    <input 
                      type="checkbox" 
                      checked={docs[key as keyof typeof docs]} 
                      onChange={() => setDocs(prev => ({ ...prev, [key]: !prev[key as keyof typeof prev] }))} 
                      className="mr-3 w-4 h-4 text-rose-600 rounded focus:ring-rose-500"
                    />
                    <span className="text-sm font-medium">{label}</span>
                  </div>
                  {docs[key as keyof typeof docs] && <span className="text-green-600 text-sm">✓</span>}
                </label>
              ))}
            </div>
            
            <div className="mt-4 p-3 bg-gray-100 rounded-lg">
              <div className="text-sm font-medium">Progress: {docsScore} / 5 documents uploaded</div>
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg text-xs text-blue-800">
              💡 OCR technology verifies that names & numbers match across all documents.
            </div>
          </MobileCard>
        )}
        ...
        {stepIdx === 2 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">{steps[2].title}</h2>
              <p className="text-sm text-gray-600 mt-1">Automated risk assessment</p>
            </div>
            
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">CCRIS</label>
                  <select 
                    value={ccris} 
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">CTOS</label>
                  <select 
                    value={ctos} 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Business Tenure</label>
                  <input 
                    type="number" 
                    value={months} 
              ...
              <label className="flex items-center p-3 bg-gray-50 border rounded-lg">
                <input 
                  ...
                  className="mr-3 w-4 h-4 text-rose-600 rounded focus:ring-rose-500"
                />
                <span className="text-sm font-medium">Bankruptcy/Insolvency Record</span>
              </label>
            </div>
            
            <div className={`mt-4 rounded-lg p-4 border ${
              decision.outcome === "PASS" ? "border-green-300 bg-green-50" :
              decision.outcome === "FAIL" ? "border-red-300 bg-red-50" :
              ...
              {decision.outcome !== "PASS" && (
                <div className="mt-2 text-xs opacity-75">
                  Cases requiring review are forwarded to loan officers with full context.
                </div>
              )}
            </div>
          </MobileCard>
        ...
        {stepIdx === 3 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">{steps[3].title}</h2>
              <p className="text-sm text-gray-600 mt-1">Get ready for your interview</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <h3 className="font-bold mb-3 text-gray-800">🤖 AI Assistant</h3>
                <ChatbotMobile />
              </div>
            </div>
          </MobileCard>
          ...
          <div className="space-y-4">
            <MobileCard>
              <div className="text-center mb-4">
                ...
                <h2 className="text-xl font-bold text-gray-800">{steps[4].title}</h2>
                <p className="text-sm text-gray-600 mt-1">Track your business cashflow</p>
              </div>
            ...
            <MobileCard>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <KPIMobile label="Sales" value={`RM ${totals.sales.toFixed(2)}`} color="green" />
                <KPIMobile label="Expenses" value={`RM ${totals.expenses.toFixed(2)}`} color="red" />
                <KPIMobile label="Net Income" value={`RM ${totals.net.toFixed(2)}`} color="blue" />
                <KPIMobile label="Risk Level" value={totals.risk} color={totals.risk === "LOW" ? "green" : totals.risk === "MEDIUM" ? "yellow" : "red"} />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Monthly Instalment: RM {instalment}</label>
                <input 
                  type="range" 
                  min={200} 
                  max={1500} 
                ...
                <div className="text-center mt-1">
                  <span className={`text-sm font-medium ${totals.surplus >= 0 ? "text-green-600" : "text-red-600"}`}>
          ...
          <div className="space-y-4">
            <MobileCard>
              <div className="text-center mb-4">
                ...
                <h2 className="text-xl font-bold text-gray-800">{steps[5].title}</h2>
                <p className="text-sm text-gray-600 mt-1">Comprehensive risk assessment report</p>
              </div>
        ...
        {stepIdx === 6 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">{steps[6].title}</h2>
              <p className="text-sm text-gray-600 mt-1">Protect yourself from fraud</p>
            </div>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Paste suspicious message:</label>
                <textarea 
                  value={sms} 
              ...
              <div className={`rounded-lg p-4 border ${
                scamFindings.length ? "border-red-300 bg-red-50" : "border-green-300 bg-green-50"
              }`}>
                {scamFindings.length ? (
                  <>
                    <div className="font-bold text-red-700 mb-2">⚠️ Scam Alert!</div>
                    <ul className="space-y-1">
                  ...
                  <div className="text-green-700 font-medium">✅ No obvious red flags detected</div>
                )}
              </div>
              
              <div className="p-3 bg-blue-50 rounded-lg">
                <div className="text-xs text-blue-800">
                  <strong>Remember:</strong> Bank Islam will never ask for your OTP/TAC or request upfront processing fees.
                </div>
              </div>
            </div>
          </MobileCard>
        ...
        {stepIdx === 7 && (
          <MobileCard>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">{steps[7].title}</h2>
              <p className="text-sm text-gray-600 mt-1">Ready for the next level?</p>
            </div>
...
function ChatbotMobile() {
  ...
  const ask = () => {
    ...
    let ans = "Saya akan cuba bantu. Sila bawa dokumen asas: bank statement 3 bulan, SSM, pelan perniagaan.";
    if (/(dokumen|document)/.test(lower)) ans = "Bawa IC, SSM, 3 bulan bank statement, contoh e‑invois, dan pelan penggunaan pembiayaan.";
    if (/(temu bual|interview|soalan)/.test(lower)) ans = "Soalan lazim: aliran tunai sebulan, kos tetap & berubah, sumber pelanggan, dan bagaimana dana akan digunakan.";
    if (/(e-?invoice|invois)/.test(lower)) ans = "e‑Invois digunakan untuk rekod jualan. Dalam app, pergi ke Pocket CFO > Tambah Invois.";
    if (/(yuran|bayaran|upfront|fee)/.test(lower)) ans = "Hati‑hati. Bank Islam tidak mengenakan yuran proses di hadapan. Elak bayar kepada individu.";
    setA(ans);
  };
...
function GraduationMobile({ totals, months }: { totals: { net: number }, months: number }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <KPIMobile label="Monthly Net" value={`RM ${totals.net.toFixed(2)}`} color="blue" />
      ...
      <div className={`rounded-lg p-4 border ${
        eligible ? "border-green-300 bg-green-50" : "border-amber-300 bg-amber-50"
      }`}>
        ...
        <div className="space-y-1 text-sm mb-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={onTimePct >= 85 ? "text-green-600" : "text-red-600"}>
              {onTimePct >= 85 ? "✓" : "✗"}
            </span>
          </div>
          <div className="flex items-center justify-between">
        ...
        <div className="font-bold text-center">
          {eligible ? (
            ...
            <span className="text-amber-700">📈 Keep Building Your Track Record</span>
          )}
        </div>
      </div>
    </div>
...
function KPIMobile({ label, value, color = "blue" }: { label: string, value: string, color?: string }) {
  const colorClasses = {
...
function EntriesListMobile({ 
  ...
  setEntries: React.Dispatch<React.SetStateAction<Array<{id: number, type: string, label: string, amount: number}>>> 
}) {
  if (entries.length === 0) {
    return (
        ...
        {entries.slice(-5).map((e) => (
          <div key={e.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-xs px-2 py-0.5 rounded-full ${
                  e.type === "sale" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                }`}>
                  {e.type}
                </span>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-sm font-bold">RM {e.amount.toFixed(2)}</span>
              <button 
